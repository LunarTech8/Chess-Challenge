...
public class BotMk03 : IChessBot
{
	// ----------------
	// DATA CODE
	// ----------------
	...
	private static readonly int START_PIECES_VALUE = PIECE_VALUES.Zip(START_PIECES_COUNTS, (x, y) => x * y).Sum();
	...
	private int CalculateMoveRating(Move move, Move prioMove)
	{
	...
	private void SortMoves(ref Move[] moves, Move prioMove)
	{
		// TODO: probably faster to store the ratings for each move instead of getting them for each compare
		Array.Sort(moves, delegate(Move moveA, Move moveB) { return CalculateMoveRating(moveB, prioMove).CompareTo(CalculateMoveRating(moveA, prioMove)); });
	}

	private float CalculateEndgameFactor(int evalPieces)
	{
		return 1F - Math.Min(1F, evalPieces / (float)START_PIECES_VALUE);
	}
	...
	private int EvaluateBoard()
	{
		...
		int evalPieces = EvaluatePieces();
		return evalPieces + EvaluateKings(CalculateEndgameFactor(evalPieces));
	}
	...
	private int MiniMax(int depth, int alpha, int beta)
	{
		...
		SortMoves(ref moves, bestMoveOverall);
	...
	public Move Think(Board currentBoard, Timer timer)  // TODO: maybe make board a global variable to not have to pass it around that often
	{
		board = currentBoard;

...
// Like BotMk21 but with array instead of GetPieceValue
public class BotMk23 : IChessBot
{
	// ----------------
	// DATA CODE
	// ----------------
	...
	private int CalculateMoveRating(Move move)
	{
		...
		if (move.IsCapture)
		{
			moveRating += PIECE_VALUES[(int)move.CapturePieceType] - PIECE_VALUES[(int)move.MovePieceType];

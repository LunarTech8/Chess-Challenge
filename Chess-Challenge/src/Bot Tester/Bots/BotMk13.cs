...
public class BotMk13 : IChessBot
{
	...
	//	- better rating of capture moves (move order)
	...
	private int CalculateMoveRating(Move move)
	{
		...
		int movePieceValue = GetPieceValue(move.MovePieceType);
		...
		int moveRating = 0;
		if (move.IsCapture)
			moveRating += CAPTURE_VALUE_FACTOR * GetPieceValue(move.CapturePieceType) - movePieceValue;  // TODO: maybe this can be improved
		if (board.SquareIsAttackedByOpponent(move.TargetSquare))
			moveRating -= movePieceValue;
		if (move.IsPromotion)

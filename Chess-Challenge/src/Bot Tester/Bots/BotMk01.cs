...
public class BotMk01 : IChessBot
{
	// ----------------
	// DATA CODE
	// ----------------
	...
	private int MiniMaxOnlyCaptures(Board board, int alpha, int beta)  // TODO: maybe this can be incooperated into MiniMax to use up less tokens
	{
		...
		{
			board.MakeMove(move);
			eval = -MiniMaxOnlyCaptures(board, -beta, -alpha);
			board.UndoMove(move);
	...
	private int MiniMax(Board board, int depth, int alpha, int beta)
	{
		...
		if (depth <= 0)
		{
			return MiniMaxOnlyCaptures(board, alpha, beta);
		}
	...
	public Move Think(Board board, Timer timer)
	{
		...
		Console.WriteLine("-------- BotMk01 --------");  // DEBUG:
